...
FrontEnd::FrontEnd()
    :ndt_ptr_(new pcl::NormalDistributionsTransform<CloudData::POINT, CloudData::POINT>()), //ndt匹配方法的对象
...
Eigen::Matrix4f FrontEnd::Update(const CloudData& cloud_data) {
    ...
    if (fabs(last_key_frame_pose(0,3) - current_frame_.pose(0,3)) + 
        ...
        fabs(last_key_frame_pose(2,3) - current_frame_.pose(2,3)) > 2.0) {  //每两米取一个关键帧(为了方便取曼哈顿距离)
        UpdateNewFrame(current_frame_);
