...
int main(int argc, char *argv[]) {
    ...
    std::shared_ptr<TFListener> lidar_to_imu_ptr = std::make_shared<TFListener>(nh, "imu_link", "velo_link");   //lidat_to_imu的坐标变换
            ...
            while (cloud_data_buff.size() > 0 && imu_data_buff.size() > 0 && gnss_data_buff.size() > 0) {
                ...
                if (d_time < -0.05) {
                    ...
                    if (!gnss_origin_position_inited) {
                        gnss_data.InitOriginPosition(); //设置gps原点
                        gnss_origin_position_inited = true;
                    }
                    gnss_data.UpdateXYZ();  //更新gps经纬度
